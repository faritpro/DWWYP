...
def filedelete(text):
    os.remove(text.text)
    bot.send_message(text.chat.id, f"Файл удалён")

def fileopen(text):
    with open(text.text, "r") as f:
        bot.send_message(text.chat.id, f"{f.read()}")

def writefile(text):
    with open(text.text.split("ARG")[0], "w") as f:
        f.write(text.text.split('ARG')[1])
        bot.send_message(text.chat.id, f"Файл изменен")

def createfile(text):
    with open(text.text, "w") as f:
        bot.send_message(text.chat.id, f"Файл создан")

def filelist(text):
    for root, dirs, files in os.walk(os.getcwd()):
        for filename in files:
            bot.send_message(text.chat.id, f"{filename}")
...
def openprocec(text):
    for i in psutil.disk_partitions():
        for rootdir, dirs, files in os.walk(i[0]):
            for file in files:
                if file == text:
...
def file_work(message):
    ...
    if message.text == "🎥 снимок":
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.add(types.KeyboardButton("Список файлов:"))
        markup.add(types.KeyboardButton("Чтение файла:"))
        markup.add(types.KeyboardButton("Измение файла:"))
        markup.add(types.KeyboardButton("Создание файла:"))
        markup.add(types.KeyboardButton("Удаление файла:"))
        markup.add(types.KeyboardButton("Назад"))
        bot.send_message(message.chat.id, 'Режим Файлов', reply_markup=markup)

    elif message.text == "Чтение файла:":
        bot.send_message(message.chat.id, 'Название файла который хотите открыть(Формате название.txt и должен лежать там где exe):')
        lastmes = 41

    elif message.text == "Измение файла:":
        bot.send_message(message.chat.id, 'Название файла который хотите изменить(Формате название.txtARGТекст для изменения и должен лежать там где exe):')
        lastmes = 42

    elif message.text == "Создание файла:":
        bot.send_message(message.chat.id, 'Название файла который хотите создать(формат название.txt сохранится там где exe):')
        lastmes = 43

    elif message.text == "Удаление файла:":
        bot.send_message(message.chat.id, 'Название файла который хотите удалить:')
        lastmes = 44
